...
{\fontsize{26.5}{34}\selectfont\color{ink}\RaggedRight
\textbf{Abstract.} This work investigates reward shaping and evidence-driven checkpoint selection for a PPO agent in a long-horizon 1v1 MOBA task. The proposed risk-aware training run introduces terminal outcome rewards, explicit tower-pressure terms, push-window diagnostics, and unsafe-dive penalties. Results show a substantial reduction in death rate, while tower conversion still lags behind the previous peak baseline. We therefore identify step-15003 as the strongest candidate for full matchup-matrix evaluation.}
...
\PosterBlock{Candidate vs. Peak Baseline}{
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}X r r r}
\toprule
\textbf{Metric} & \textbf{Candidate} & \textbf{Baseline} & \textbf{Delta}\\
\midrule
...
\vspace{0.72cm}
Win rate tracks tower pressure, self-tower survival, economy, kills, and outgoing hero damage. Death is no longer the main bottleneck because the risk-aware run has already compressed deaths into a low range.
}
...
\Status{orange}{WARN} & win rate below 0.84; tower pressure below the peak baseline\\
...
\vspace{0.72cm}
\textbf{Meaning.} The gate failure is actionable: run the missing evidence package before making a final model claim.
}
...
\begin{itemize}
  \item Freeze \textbf{step-15003} as the candidate checkpoint.
  ...
  \item Compare full reward, no-window, no-terminal, and death-only-risk ablations.
\end{itemize}
}
\end{minipage}
...
\node[
  ...
  \vspace{0.38cm}
  {\fontsize{20.5}{26}\selectfont\color{ink}
  Deaths dropped from the late-training baseline 3.09 to 0.47 at the selected checkpoint. The agent learned safety.}
};
\node[
  rounded corners=3mm,
  fill=paleWarn,
  ...
  \vspace{0.38cm}
  {\fontsize{20.5}{26}\selectfont\color{ink}
  Enemy tower HP remains 2129, worse than the peak baseline. Safety is not yet converted into tower destruction.}
};
\node[
  rounded corners=3mm,
  fill=paleBlue,
  ...
  \vspace{0.38cm}
  {\fontsize{20.5}{26}\selectfont\color{ink}
  The full 9-matchup matrix with push-window and unsafe-dive evidence decides whether the candidate generalizes.}
};
\end{tikzpicture}
...
\node[
  ...
  {\fontsize{34}{41}\selectfont\bfseries\color{ink}Takeaway}\par
  \vspace{0.52cm}
  {\fontsize{24.5}{31}\selectfont\color{ink}
  Risk-aware reward shaping can sharply reduce deaths, but stable MOBA success still depends on converting safety into tower destruction. The next decisive evidence is the full matchup matrix for step-15003.}
};
\end{tikzpicture}
