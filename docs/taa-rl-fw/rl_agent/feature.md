---
title: 特征处理
source: https://tencentarena.com/docs/p-edu-hok1v1/64.3.1/taa-rl-fw/rl_agent/feature
saved_at: 2026-06-22
---
...
将环境返回的观测数据转换成ObsData类型数据。 很多情况下，特征工程包含了大量的数值处理、数据转换和领域知识，我们建议将大量的特征处理代码在`<智能体文件夹>/feature/preprocessor.py`文件中实现，然后由于`observation_process`进行调用。
...
> **💡 补充说明**：当环境返回的数据较为复杂时，将所有奖励逻辑塞进一个 `reward_shaping` 函数会导致代码可读性和可维护性下降。此时开发框架会将奖励函数**封装成一个独立的类**，在类内部统一管理子奖励项的计算、状态缓存与权重组合，再调用该类的接口完成最终 Reward 的计算。具体实现细节请参考开发指南中**智能体详述**章节的奖励函数实现部分。
